...
// It comes from print.go in the go/doc/comment package.
func blankBefore(x comment.Block) bool {
	if x, ok := x.(*comment.List); ok {
		return x.BlankBefore()
	}
	return true
}

// A cmtFormatter holds the state needed for formatting a comment with wrapping
// and a style closer to that of Go's text format for comments.
type cmtFormatter struct {
...
func (p *cmtFormatter) block(out *bytes.Buffer, x comment.Block) {
	switch x := x.(type) {
	default:
		fmt.Fprintf(out, "?%T\n", x)
	...
	case *comment.List:
		...
		for i, item := range x.Items {
			...
			for i, blk := range item.Content {
				...
				p.text(out, fourSpace, blk.(*comment.Paragraph).Text)
			}
		}
	}
}
...
func (p *cmtFormatter) text(out *bytes.Buffer, indent string, x []comment.Text) {
...
func (p *cmtFormatter) oneLongLine(out *strings.Builder, x []comment.Text) {
	for _, t := range x {
		switch t := t.(type) {
		case comment.Plain:
